...
// equivalents). Copies the zignapi Zig sources (the monorepo's `native/`) into
...
if (!existsSync(join(pkgRoot, "dist", "cli.js"))) {
  console.error("prepack: dist/ is missing — run `pnpm --filter zignapi build` first");
  process.exit(1);
}

if (!existsSync(join(src, "build.zig.zon"))) {
  console.error(`prepack: cannot find zignapi sources at ${src}`);
  process.exit(1);
}
